        ...
        <div className="flex items-center gap-2">
          {activeConversation?.mode === 'ai' ? (
            ...
            <button 
              ...
              className="flex items-center justify-center gap-1.5 w-8 h-8 sm:w-auto sm:px-3 sm:py-1.5 bg-slate-50 hover:bg-[rgba(255,255,255,0.08)] border border-slate-200 text-[#E4E4E7] text-[13px] font-medium rounded-md transition-all"
              title="Return to AI"
            >
          ...
          <button 
            ...
            className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 bg-slate-50 hover:bg-[rgba(255,255,255,0.08)] border border-slate-200 text-[#E4E4E7] text-[13px] font-medium rounded-md transition-all">
              ...
              <div className="max-w-[85%] w-full mb-4 flex justify-start">
                <div className="flex gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-[10px] text-slate-400 mt-1.5 font-medium text-left flex items-center gap-1">
                      {m.time}
          ...
          <>
            <div className="flex flex-wrap items-center gap-2 mb-2 px-1">
              ...
              <button 
                ...
                className="flex items-center gap-1.5 px-3 min-h-[36px] text-xs sm:text-sm font-semibold rounded-lg text-slate-400 hover:bg-slate-100 hover:text-[#FAFAFA] transition-all relative"
              >
            ...
            {showSavedReplies && (
              <div className="absolute bottom-[100px] right-4 lg:right-[300px] mb-2 w-[calc(100vw-2rem)] sm:w-80 bg-[#18181B] rounded-xl shadow-[0_16px_40px_rgba(0,0,0,0.5)] border border-slate-200 overflow-hidden z-20 animate-in fade-in slide-in-from-bottom-2">
                <div className="px-3 py-2 border-b border-slate-200/80 bg-white flex justify-between items-center">
                  <span className="text-xs font-bold text-[#FAFAFA]">Saved Replies (Templates)</span>
                  <button onClick={() => setShowSavedReplies(false)} className="text-slate-400 hover:text-[#FAFAFA]">
                ...
                <div className="max-h-60 overflow-y-auto">
                  {savedReplies.map(reply => (
                    <button 
                      ...
                      className="w-full text-left px-4 py-3 hover:bg-slate-50 border-b border-[rgba(255,255,255,0.02)] transition-colors flex items-start gap-3 group"
                    >
                      <div className="bg-slate-100 text-slate-400 text-[10px] font-mono px-1.5 py-0.5 rounded border border-slate-200 shrink-0 mt-0.5">
                        {reply.shortcut}
                      </div>
                      <div>
                        <p className="text-xs font-semibold text-[#E4E4E7] mb-0.5 group-hover:text-[#4F46E5]">{reply.title}</p>
                        <p className="text-[11px] text-slate-400 line-clamp-2">{reply.content}</p>
                      </div>
                    </button>
                  ))}
                </div>
                <div className="p-2 border-t border-slate-200/80 bg-white">
                  <button 
                    onClick={() => {
                      setShowSavedReplies(false);
                      setShowCreateReplyModal(true);
                    }}
                    className="w-full py-2 bg-slate-50 border border-slate-200 text-xs font-bold text-[#E4E4E7] rounded-lg hover:bg-[rgba(255,255,255,0.06)] hover:text-[#4F46E5] transition-colors flex items-center justify-center gap-2"
                  >
                ...
                <div className="flex gap-1">
                  ...
                  <button 
                    ...
                    className={`w-11 h-11 flex items-center justify-center rounded-lg transition-colors ${
                      isPrivateNote ? 'text-yellow-600 hover:bg-yellow-500/20' : 'text-slate-400 hover:bg-slate-100 hover:text-[#FAFAFA]'
                    }`}
                  >
            ...
            <div className="flex justify-between items-center mt-2 px-1">
              <span className="text-[11px] text-slate-400 font-medium flex items-center gap-1.5">
                <kbd className="px-1.5 py-0.5 bg-slate-100 rounded-[4px] border border-slate-200 text-[9px] font-sans">Enter</kbd> to send, <kbd className="px-1.5 py-0.5 bg-slate-100 rounded-[4px] border border-slate-200 text-[9px] font-sans">Shift + Enter</kbd> for new line
              </span>
              <div className="relative">
                ...
                {showArticles && (
                  <div className="absolute bottom-full right-0 mb-2 w-[calc(100vw-2rem)] sm:w-72 bg-[#18181B] rounded-xl shadow-[0_16px_40px_rgba(0,0,0,0.5)] border border-slate-200 overflow-hidden z-10 animate-in fade-in slide-in-from-bottom-2">
                    <div className="px-3 py-2 border-b border-slate-200/80 bg-white flex justify-between items-center">
                      <span className="text-xs font-bold text-[#FAFAFA]">Knowledge Base</span>
                      <button onClick={() => setShowArticles(false)} className="text-slate-400 hover:text-[#FAFAFA]">
                    ...
                    <div className="max-h-60 overflow-y-auto">
                      {kbArticles.map(article => (
                        <button 
                          ...
                          className="w-full text-left px-3 py-2.5 hover:bg-slate-50 border-b border-[rgba(255,255,255,0.02)] transition-colors group"
                        >
                          <p className="text-xs font-semibold text-[#E4E4E7] mb-0.5 group-hover:text-[#4F46E5]">{article.title}</p>
                          <p className="text-[10px] text-slate-400 line-clamp-1">{article.content}</p>
                        </button>
                      ))}
                    </div>
                  </div>
